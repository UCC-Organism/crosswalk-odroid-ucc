...
def Prepare(options, sanitized_name):
  ...
  if(options.enable_auto_update):
    src_app = 'app_src_auto_update'
    print "------- auto-update configuration"
  shutil.copytree(src_app, sanitized_name)
  shutil.rmtree(os.path.join(sanitized_name, 'src'))
  src_root = os.path.join(src_app, 'src', 'org', 'xwalk', 'app', 'template')
  ...
  if(options.enable_auto_update):
    update_apk = os.path.join(src_app, 'src', 'com', 'autoupdateapk', 'AutoUpdateApk.java')
    silent_update_apk = os.path.join(src_app, 'src', 'com', 'autoupdateapk', 'SilentAutoUpdate.java')
    if not os.path.isfile(update_apk) or not os.path.isfile(silent_update_apk):
      print ('Please make sure that the autoupdateapk java files does exist.')
      sys.exit(7)
    root_path_update_apk = os.path.join(sanitized_name, 'src', 'com', 'autoupdateapk')
    if not os.path.exists(root_path_update_apk):
      os.makedirs(root_path_update_apk)
    dest_update_apk = os.path.join(root_path_update_apk, 'AutoUpdateApk.java')
    dest_silent_update_apk = os.path.join(root_path_update_apk, 'SilentAutoUpdate.java')
    shutil.copyfile(update_apk, dest_update_apk)
    shutil.copyfile(silent_update_apk, dest_silent_update_apk)
...
def CustomizeXML(options, sanitized_name):
  ...
  if options.enable_auto_update:
    receiver_name = activity_name + '$' + 'UCCLauncherReceiver'
    EditElementAttribute(xmldoc, 'receiver', 'android:name', receiver_name)
...
def main():
  ...
  parser.add_option('--enable-auto-update', action='store_true',
                    dest='enable_auto_update', default=False,
