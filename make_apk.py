...
def Customize(options):
  ...
  cmd = ['python', 'customize.py', package,
          ...
          extensions_list, orientation, auto_update]
...
def Execution(options, sanitized_name):
  ...
  if options.mode == 'shared':
    ...
    if options.enable_auto_update:
      print "*** renaming apk to correspond to auto-update-apk ***\n"      
      dst_file = '%s-%s.apk' % (options.name, options.app_version)
    else:
    ...
    if options.arch:
      ...
      if options.enable_auto_update:
        print ('The Crosswalk embedded APK of web application "%s" for '
             'platform %s was generated successfully at %s-%s.apk.'
             % (sanitized_name, options.arch, sanitized_name, options.app_version))        
      else:
        print ('The Crosswalk embedded APK of web application "%s" for '
      ...
      for arch in valid_archs:
        if os.path.isfile(os.path.join('native_libs', arch, 'libs',
          ...
          if options.enable_auto_update:
            apk_str += '%s-%s.apk' % (sanitized_name, options.app_version)
          else:
...
def main(argv):
  ...
  group.add_option('--enable-auto-update', action='store_true',
